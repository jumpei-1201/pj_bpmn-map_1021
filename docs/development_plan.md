...
### フェーズ5: テスト・ドキュメント
1. 統合テスト/E2E
   - Cypress/Playwright で主要ユーザーフロを自動化
2. 性能・回帰テスト
   - 大規模BPMNデータセットで負荷検証
3. ドキュメント整備
...
## 9. 仮想環境・インフラ整備
- ローカル開発では Node.js 20.x + Vite を採用し、ホットリロードでBPMNビューアを確認できるようにする。
- `Dockerfile` を用意し、`npm run build` による成果物を `vite preview` で提供する軽量なコンテナイメージを構築。
- CI/CD では Docker イメージをビルドし、E2E テスト用に `docker-compose` または Kubernetes 上で再利用できるようにアーティファクトを共有。
- 本番運用は静的ファイル配信を基本とし、CDN/オブジェクトストレージ (S3, Cloud Storage) への配置を想定。
- 今後の拡張として、バックエンド API 連携が必要になった際は BFF (Backend for Frontend) 層を Node.js で構築し、Docker Compose 上でフロント/バックの統合検証を行う。
